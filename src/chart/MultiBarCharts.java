...
public class MultiBarCharts
{
	public static JPanel createCategoryBar(String chartName,String xName,String yName,Map<String, Map<String, Double>> result) {
	...
	private static JFreeChart createChart(String chartName,String xName,String yName,CategoryDataset paramCategoryDataset)
	{
		...
		renderer.setMaximumBarWidth(0.03);		// 柱子最大宽度
		renderer.setItemMargin(0); 				// 组内柱子间隔为组宽为0
	...
	private static CategoryDataset createDataset(Map<String,Map<String,Double>> result)
	{
		DefaultCategoryDataset localDefaultCategoryDataset = new DefaultCategoryDataset();
		Set<String> algoNames = result.keySet();
		for(String algoName : algoNames){
			Map<String,Double> dataMap = result.get(algoName);
			Set<String> categorys = dataMap.keySet();
			for(String category : categorys){
				localDefaultCategoryDataset.addValue(dataMap.get(category), algoName, category);
			}
		}
