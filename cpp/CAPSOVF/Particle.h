...
public:
	...
	* 初始化静态运行参数
	*/
	static void initStaticParams(int generation,double wMax,double wMin,double c1Begin,double c1End,double c2Begin,double c2End,double c3Begin,double c3End,double f1,double f2);
...
public:
	...
	Particle():pos(dims,posmin),v(dims,0),pbest(dims,posmax),force(dims,0){
		fitness = 0;
		pbest_fitness = 0;
	}
	...
	*/
	void evaluate() {
		...
		EvaluateCalculator::getInstance().updateVirtualForce(nodes,force);
	...
	*/
	void updatePosSpeed() {
		for (int i = 0; i < dims; ++i) {
			v[i] = w * v[i] + c1 * rnd.frandom() * (pbest[i] - pos[i]) + c2 * rnd.frandom() * (gbest[i] - pos[i]) + c3 * force[i];
			if (v[i] > vmax) {
				v[i] = vmax;
			}
			if (v[i] < vmin) {
...
double Particle::c3Begin = 0.5;
double Particle::c3End = 0.1;
...
void Particle::initStaticParams(int generation,double wMax,double wMin,double c1Begin,double c1End,double c2Begin,double c2End,double c3Begin,double c3End,double f1,double f2){
	...
	Particle::c3Begin = c3Begin;
	Particle::c3End = c3End;
	Particle::f1 = f1;
	Particle::f2 = f2;
	...
	Particle::c3 = Particle::c3Begin + (Particle::c3End - Particle::c3Begin) / generation;
}
...
void Particle::updatePSOParamValues(int gen){
	...
	Particle::c3 = Particle::c3Begin + gen * (Particle::c3End - Particle::c3Begin) / generation; 
}
